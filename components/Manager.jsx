...
const Manager = () => {
    ...
    useEffect(() => {
        const effect=async ()=>{
            ...
            let compileData = await data.json();
            if (compileData.length > 0) {
                setpasswordArray(compileData);
            }}
            effect();
    }, []);
    ...
    const handleSave = async () => {
        if (form.site.length > 3 && form.username.length > 3 && form.password.length >= 8) {
            ...
            },body:JSON.stringify(ref2.current.item)});
            ref2.current = false;
        }
            let obj={ ...form, id: uuidv4() };
            let response=  await fetch('http://localhost:3000', {
                method: "POST", headers: {
                    'Content-Type': 'application/json'
                }, body: JSON.stringify(obj)
            });
            let compileData = await response.json();
            setpasswordArray(compileData);
    ...
    const handleEdit = (item) => {
        ...
        ref2.current= {item,flag:true};
        setpasswordArray(newArray);
    }

    const handlleDelete = async (item) => {
        let conf = confirm("Do you really want to Delete this password?");
        if (conf) {
            let response =  await fetch('http://localhost:3000',{method:"DELETE",headers:{
                "Content-Type":"application/json"
            },body:JSON.stringify(item)});
            let compile= await response.json();
            setpasswordArray(compile);
            ...
            <div className=" container md:w-[60%] w-[95%] mx-auto py-4 flex items-center flex-col" >
                ...
                <div className="items-center input flex flex-col p-2 w-full">
                    <div className='w-full'>
                        <input type="text" value={form.site} onChange={handleChange} className='bg-white my-2 w-full px-3 rounded-full py-1 h-8' placeholder='website url' name="site" id="site" /></div>
                    <div className="md:flex w-full gap-2" >
                        <input type="text" value={form.username} onChange={handleChange} className='bg-white my-2 px-3 rounded-full w-full py-1 h-8' placeholder='username' name="username" id="username" />
                        <div className="password flex relative md:w-2/3">
                            <input type="text" ref={ref1} value={form.password} onChange={handleChange} className='bg-white my-2 px-3 rounded-full w-full py-1 h-8' placeholder='password' name="password" id="password" /><img src="./icons/eye.png" alt="" className='w-6 absolute right-3 top-3 cursor-pointer' onClick={handlePasswd} /></div>
                    </div>
